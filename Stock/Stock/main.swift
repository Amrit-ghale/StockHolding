...
//let nsnumberformatterstyle = NumberFormatter.Style(rawValue: <#UInt#>)
//print("Hello, World!")

//formatter.numberStyle = NumberFormatter.CurrencyStyle //swathi

let formatter = NumberFormatter() //formatter constant as NumberFormatter() and is fully dependent on the locale settings of the device
...
func addStocks( val1 :Int, type : String) // to add stoks to the stocklist array
{
    ...
    let val2 = validOptionFloat(start: 0.0)
    print("Please enter Current price of stock")
    let val3 = validOptionFloat(start: 0.0)
    print("Please enter number of stock")
    let val4 = validOptionString( start: 1,end: 1000)
    if (type == "local"  )
    {
    ...
    else
    {
        ...
        let val5 = validOptionFloat(start: 0.0)
...
{
    print("Stock Number",
          addSpaces(width: 18, str: "Stock Name",var1: 0),
          addSpaces(width: 18, str: "Number of Stocks",var1: 0))
    print(String (repeating: Character("="), count :50))
    for i in 0...stocklist.count-1
    {
       if (val == 1)
       {
                 ...
                 if (values == stocklist[i].companyName)
                    {
                       print(String (format: "%5d%@%10d", key ,
                                     addSpaces(width: 26,str: values,var1: 0),
    ...
    if (val == 1)
    {
        while (val1 == -1)
        {
            val1 = validOptionString(start: 1,end: localStocks.count)
            if (checkStock(val: val1,stocktype: 1) != true)
            {
    ...
    else
    {
        while (val1 == -1)
        {
            val1 = validOptionString(start: 1,end: forgnStocks.count)
            if (checkStock(val: val1,stocktype: 2) != true)
            {
                print("Please enter above provided Stock Number only!")
                val1 = -1
            }
...
repeat
{
    ...
    {
        showMenu()
        option = validOptionString(start: 1,end: 9)
        switch option
        {
            ...
            let a = validOptionString(start: 1,end: 2)
                    if (a == 1)
                    {
                        displayLocalStocks()
                        print("Chose stock number from 1 - \(localStocks.count)" )
                        let val1 = validOptionString(start: 1,end: localStocks.count)
                        addStocks (val1: val1 , type : "local")
                        diplaystocks()
                    }
                    if (a == 2)
                    {
                       displayForgnStocks()
                        ...
                        let val1 = validOptionString(start: 1,end: forgnStocks.count)
                        addStocks (val1: val1 , type : "foriegn")
                        diplaystocks()
                    }
              case  2:// for removing an existing stock
                    if (validateStock()  == true)
                    {
                        ...
                        if (a == 1)
                        {
                            ...
                            {
                                if (checkStock (val: i, stocktype: 1) == true)
                                {
                                     counter = counter + 1
                                }
                            }
                        ...
                        if (a == 2)
                        {
                            ...
                            {
                                if (checkStock (val: i, stocktype: 2) == true)
                                {
                                    counter = counter + 1
                                }
                            }
          ...
          //  stocklist.sortInPlace({ $0.purchaseSharePrice < $1.purchaseSharePrice })
            ...
            print(stocklist.sorted(by: { $0.purchaseSharePrice < $1.purchaseSharePrice })) // swathi
            ...
            displayIndividualStock(index: temp)
              case  7://Diplay stock from A-Z in ascending order of company name
                 //     stocklist.sortInPlace({ $0.companyName < $1.companyName })
                    stocklist.sorted(by: { $0.companyName < $1.companyName }) //swathi
                      diplaystocks()
            
              case  8://Diplay stocks from lowest to highest purchase price
                     // stocklist.sortInPlace({ $0.purchaseSharePrice < $1.purchaseSharePrice })
            stocklist.sorted(by: { $0.purchaseSharePrice < $1.purchaseSharePrice }) //swathi
                      diplaystocks()
              case  9: break
              default: break
            }
